  ...
  for (unsigned long i = 0; i < dir->entry_count; i++) {
    ...
    if (entries[i].original_length) {
      if (entries[i].length != -1) {
        ...
        read_unobfuscate(fd, base_offset + entries[i].offset, buff, len, seed);
      ...
      if (entries[i].offset) {
        process_dir(fd, hdr, toc_buff, entries[i].offset, out_filename, seed);
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  DXAHDR hdr;
  read_unobfuscate(fd, 0, &hdr, sizeof(hdr), seed);
  ...
  read_unobfuscate(fd, hdr.toc_offset, toc_buff, toc_len, seed);

  process_dir(fd, hdr, toc_buff, 0, ".", seed);
